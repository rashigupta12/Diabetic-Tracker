...
import { getSession, requireAuth, AuthUser } from '@/lib/auth';
...
export async function GET(request: NextRequest) {
  try {
      ...
      const reading = await db
        ...
        .where(
          and(
            ...
            eq(bloodSugarReadings.userId, auth.user.id)
          )
        )
        .limit(1);
    ...
    const conditions = [eq(bloodSugarReadings.userId, auth.user.id)];
